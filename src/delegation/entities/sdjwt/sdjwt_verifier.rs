...
impl Verifier<Jwk> for SdJWTVerifier {
    ...
    fn verify_verifiable_presentation(
        ...
        let vp: VerifiablePresentation<SdJWTDelegationCredential> =
            VerifiablePresentation::<SdJWTDelegationCredential>::from_signed_jwt(
                signed_jwt, &ecc_pk,
            )?;

        let dc = vp.credential();
        let permissions = dc.permissions().clone();
        let permission_salts = dc.permission_salts().clone();
        let hashes = dc.hashes().clone();
...
impl SdJWTVerifier {
    ...
    fn verify_delegation<D: SdJWTDelegation>(
        &self,
        delegation: &D,
        issuer: &String,
        hashes: &Vec<String>,
...
mod tests {
    ...
    fn verify_vp() -> Result<(), String> {
        ...
        let issuer: SdJWTIssuer = SdJWTIssuer::new(id, issuer_dlt.clone(), holder_dlt.clone())?;
        ...
        let issuer: SdJWTIssuer =
            SdJWTIssuer::new(id.clone(), issuer_dlt.clone(), holder_dlt.clone())?;
        ...
        let verifier = SdJWTVerifier::new(issuer_dlt.clone(), holder_dlt.clone())?;
