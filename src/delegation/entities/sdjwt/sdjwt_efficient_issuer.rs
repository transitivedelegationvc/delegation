...
pub struct SdJWTEfficientIssuer {
    id: String,
    issuer_jwk: Jwk,
    holder_jwk: Jwk,
}

impl Issuer<Jwk, SdJWTEfficientDelegationCredential> for SdJWTEfficientIssuer {
    ...
    fn new(id: String, issuer_dlt: DLTSim<Jwk>, holder_dlt: DLTSim<Jwk>) -> Result<Self, String> {
        ...
        issuer_dlt
            .borrow_mut()
            .insert(id.clone(), issuer_jwk.clone());
    ...
    fn issue_delegation_verifiable_credential(
        ...
        optional_issuer_vc: Option<VerifiableCredential<SdJWTEfficientDelegationCredential>>,
    ) -> Result<VerifiableCredential<SdJWTEfficientDelegationCredential>, String> {
        ...
        match optional_issuer_vc {
            ...
            None => {
                ...
                let hierarchy: Vec<SdJWTEfficientDelegator> = vec![];
                let dc = SdJWTEfficientDelegationCredential::new(
                    delegatee_id,
                    Some(hashes),
                    iat,
                    exp,
            ...
            Some(issuer_vc) => {
                ...
                let mut permission_salts = issuer_dc.permission_salts().clone();
                ...
                // ones
                    ...
                    for i in removable_indices.iter().rev() {
                        permission_salts.remove(*i);
                    }
                }

                let hashes = issuer_dc.hashes().clone();
                let signature = sdjwt_utils::generate_signature(
                    &self.issuer_jwk,
                    &delegatee_id,
                    &iat,
                ...
                let issuer_delegator = SdJWTEfficientDelegator::new(
                ...
                let result_dc = SdJWTEfficientDelegationCredential::new(
                    delegatee_id,
                    hashes,
                    iat,
                    exp,
...
mod tests {
    ...
    fn issue_vc() -> Result<(), String> {
        ...
        let issuer: SdJWTEfficientIssuer =
            SdJWTEfficientIssuer::new(id, acc_sim.clone(), ecc_sim.clone())?;
