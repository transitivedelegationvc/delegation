...
use crate::delegation::credentials::sdjwt::sdjwt_efficient_delegator::SdJWTEfficientDelegator;
...
pub struct SdJWTEfficientDelegationCredential {
    #[serde(rename = "sub")]
    delegatee_id: String,
    #[serde(rename = "has")]
    ...
    hierarchy: Vec<SdJWTEfficientDelegator>,
}

impl SdJWTEfficientDelegationCredential {
    ...
    pub fn new(
        ...
        signature: String,
        hierarchy: Vec<SdJWTEfficientDelegator>,
    ) -> Result<SdJWTEfficientDelegationCredential, String> {
        Ok(SdJWTEfficientDelegationCredential {
            delegatee_id,
            hashes,
            iat,
            exp,
    ...
    pub fn hierarchy(&self) -> &Vec<SdJWTEfficientDelegator> {
        &self.hierarchy
    }
...
impl SdJWTEfficientDelegation for SdJWTEfficientDelegationCredential {
...
impl Credential for SdJWTEfficientDelegationCredential {
    ...
    fn from_map(map: Map<String, Value>) -> Result<Self, String> {
        match serde_json::from_value::<SdJWTEfficientDelegationCredential>(Value::Object(
            map.clone(),
        )) {
    ...
    fn from_string(str: String) -> Result<Self, String> {
        match serde_json::from_str::<SdJWTEfficientDelegationCredential>(&str) {
            Ok(credential) => Ok(credential),
            Err(err) => Err(format!(
    ...
    /// Generates a serde_json Map<String, Value> object from the current
    /// SdJWTEfficientDelegationCredential instance.
    ///
    /// # Returns
    ...
    fn to_map(&self) -> Result<Map<String, Value>, String> {
        let map_value = match ::serde_json::to_value(&self) {
            ...
            Err(err) => {
                return Err(format!(
                    "Failed to serialize SdJWTEfficientDelegationCredential to map [{err}]"
                ));
            }
        };
    ...
    fn to_string(&self) -> Result<String, String> {
        match serde_json::to_string(&self) {
            Ok(str) => Ok(str),
            Err(err) => Err(format!(
                "Failed to serialize SdJWTEfficientDelegationCredential to json string [{err}]"
            )),
        }
    }
    ...
    fn is_empty(&self) -> bool {
        match &self.hashes {
            None => self.permissions.is_empty(),
            Some(hashes) => self.permissions.is_empty() || hashes.is_empty(),
        }
    }
}

impl Display for SdJWTEfficientDelegationCredential {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match Credential::to_string(self) {
            Ok(result) => write!(f, "{}", result),
            Err(e) => {
                eprintln!(
                    "SdJWTEfficientDelegationCredential serialization failed: {}",
                    e
                );
                Err(std::fmt::Error)
            }
...
mod tests {
    ...
    fn parse_dc() -> Result<(), String> {
        ...
        for (name, dc) in names.iter().zip(dcs.iter()) {
            let dc: SdJWTEfficientDelegationCredential = match serde_json::from_str(dc) {
                Ok(dc) => dc,
                Err(err) => {
                    return Err(format!(
            ...
            let dc = SdJWTEfficientDelegationCredential::from_map(dc_map)?;
