...
// TODO: Documentation
impl<E: Pairing> Benchmark for OurBenchmark<E> {
    fn new(delegators_size: usize) -> Result<OurBenchmark<E>, String> {
        let issuer_dlt: DLTSim<DLTSimAccEntry<E>> = new_dlt_sim();
        ...
        for i in 0..delegators_size {
            ...
            let delegator = OurIssuer::new(id, issuer_dlt.clone(), holder_dlt.clone())?;
    ...
    fn iterate_over_delegators(
        ...
        let verifier: OurVerifier<E> =
            OurVerifier::new(self.issuer_dlt.clone(), self.holder_dlt.clone())?;
